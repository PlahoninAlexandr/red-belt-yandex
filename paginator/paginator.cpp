...
public:
    Paginator(Iterator b, Iterator e, size_t s) : first(b), last(e), page_size(s) {
        if (first != last) {
            for (Iterator it = first; it < last; it += page_size) {
                if (it + page_size < last) {
                    pages.push_back(IteratorRange<Iterator>(it, it + page_size));
                }
                else {
                    pages.push_back(IteratorRange<Iterator>(it, last));
                    break;
                }
            }
        }
